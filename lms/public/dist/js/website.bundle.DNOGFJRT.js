(() => {
  ...
  var require_common_functions = __commonJS({
    "../lms/lms/public/js/common_functions.js"(exports) {
      frappe.ready(() => {
        setup_file_size();
        pin_header();
        $(".enroll-in-course").click((e2) => {
          enroll_in_course(e2);
        });
        $(".notify-me").click((e2) => {
          notify_user(e2);
      ...
      var pin_header = () => {
        ...
        if (el) {
          const observer = new IntersectionObserver(
            ([e2]) => e2.target.classList.toggle("is-pinned", e2.intersectionRatio < 1),
            { threshold: [1] }
          );
          observer.observe(el);
        }
  ...
  var hide_profile_and_dashboard_for_guest_users = () => {
    if (frappe.session.user == "Guest") {
      let links = $(".nav-link").filter(
        (i2, elem) => $(elem).text().trim() === "My Profile" || $(elem).text().trim() === "Dashboard"
      );
      links.length && links.each((i2, elem) => $(elem).addClass("hide"));
    }
  };

  // ../lms/lms/public/js/website.bundle.js
  var import_common_functions = __toESM(require_common_functions());
  ...
  frappe.ui.RealtimeChart = class RealtimeChart extends frappe.Chart {
    constructor(element, socketEvent, maxLabelPoints = 8, data) {
      ...
      this.update_chart = function(label, data2) {
        ...
        this.addDataPoint(__(label), data2);
      };
    }
  };
...
//# sourceMappingURL=website.bundle.DNOGFJRT.js.map
